...
function HashedTransferForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log("Mulai proses transfer");
      ...
      if (!contract.methods[funcTransfer]) {
        alert("❌ Metode tidak ditemukan di smart contract.");
        setSending(false); // pastikan tombol aktif kembali
        return;
      }

      console.log("Sebelum kirim ke smart contract");
      await contract.methods[funcTransfer](
        selectedGanache,
        amountInWei,
        hexHash
      ).send({ from: account });
      console.log("Setelah kirim ke smart contract");
      ...
      const transaksiCollectionRef = collection(
        db,
        "user",
        userDocId,
        "transaction"
      ...
      console.log("Sebelum simpan ke Firestore");
      ...
      console.log("Setelah simpan ke Firestore");
    ...
    <div className="hashed-transfer-container">
      ...
      {lastHash && (
        <div className="hash-result">
          <p>
            <strong>Hash ({hashType}):</strong> {lastHash}
          </p>
          <p>
            <strong>Waktu Hash:</strong> {hashTime} ms
          </p>
        </div>
      )}
    </div>
